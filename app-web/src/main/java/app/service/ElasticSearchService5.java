...
import app.test.SvmUtil;
import sys.util.StringUtil;

@Service
public class ElasticSearchService5 {

	private static final Logger LOG = LoggerFactory.getLogger(ElasticSearchService5.class);

	// private final String host = "localhost";
	private final String host = "192.168.1.96";
	...
	public Set<String> analyze2(TransportClient client, String word) {
		...
		for (AnalyzeToken at : ikTokenList) {
			// LOG.debug("[{}][{}]", at.getPosition(), at.getTerm());
			String st = at.getTerm();
			// if (st.length() == 2 && st.matches("[\\u4e00-\\u9fa5]+"))
			// result.add(st);
			String s = SvmUtil.parseWord(st);
			if (!"".equals(s))
				result.add(s);
		}
		return result;
