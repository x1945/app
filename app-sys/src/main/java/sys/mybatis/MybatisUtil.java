...
public final class MybatisUtil {
	...
	public static final long CACHE_TIME = 60000 * 5; // 60秒*5 = 5分鐘
	public static final long CACHE_MINUTE = 60000; // 1分鐘
	public static final long CACHE_3MINUTE = 60000 * 3; // 3分鐘
	public static final int CACHE_SIZE = 1024; // 1024筆
	...
	public static Class<?> getrResolveType(Class<?> clazz) {
		Class<?>[] types = GenericTypeResolver.resolveTypeArguments(clazz, BaseDao.class);
	...
	public static String parseParameter(BoundSql boundSql) {
		...
		if (parameterObject instanceof HashMap) {
			...
			for (ParameterMapping pm : ParameterList) {
				String key = StringUtil.trim(pm.getProperty());
				String value = StringUtil.trim(tempMap.get(key));
				result.append("&").append(key).append("=").append(value);
			}
		} else if (parameterObject instanceof String) {
			List<ParameterMapping> ParameterList = boundSql.getParameterMappings();
			if (ParameterList.size() == 0) {
				result.append("&primaryKey").append("=").append(StringUtil.trim(parameterObject));
			} else {
				for (ParameterMapping pm : ParameterList) {
					String key = StringUtil.trim(pm.getProperty());
					String value = StringUtil.trim(parameterObject);
					result.append("&").append(key).append("=").append(value);
				}
			}
		}
	 ...
	 * @return
	 */
	public static String zipTrim(String input) {
		if (input == null)
			return "";
